...
fig_count_historic <- ggplot(MHW_cat_daily_mean, aes(x = t, y = cat_prop)) +
  ...
  labs(y = "Average MHW occurrence", x = NULL) +
  coord_cartesian(expand = F) +
...
fig_ALL_cap <- grid::textGrob(paste0("MHW category summaries: 1982 - 2019",
                                     "\nNOAA OISST; Climatogy period: 1982 - 2011"), 
                              x = 0.01, just = "left", gp = grid::gpar(fontsize = 14))
fig_ALL_full <- ggpubr::ggarrange(fig_ALL_cap, fig_ALL_historic, heights = c(0.15, 1), nrow = 2)
ggsave(fig_ALL_full, filename = paste0("figures/MHW_cat_historic.png"), height = 4, width = 12)
